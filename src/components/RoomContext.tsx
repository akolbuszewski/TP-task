...
export const RoomsProvider: React.FC<{children: ReactNode}> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        setPaginatedRooms(rooms.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE));
    }, [sortCriteria])
    ...
    const sortRooms = (criteria: 'price' | 'name') => {
        const sorted = [...rooms];
        if (criteria === 'price') {
            sorted.sort((a, b) => (a?.currentPrice?.value || a?.price?.value || Infinity) - (b?.currentPrice?.value || b?.price?.value || Infinity));
    ...
    const checkAvailability = async (id: number) => {
        ...
        setRooms((prevRooms) =>
            prevRooms.map(room =>
                room.id === id ? { ...room, availabilityStatus: data.availabilityStatus, currentPrice: data.price  } : room
            )
        );
    };
    ...
    return (
        <RoomsContext.Provider value={{ rooms, sortRooms: setSortCriteria, checkAvailability, currentPage, numberOfPages, paginatedRooms, setPage: setCurrentPage, book }}>
            {children}
